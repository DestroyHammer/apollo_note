...
bool Scheduler::CreateTask(std::function<void()>&& func,
  ...
  * 为输入的func创建协程
  * dispatchTask：
  * 1将该协程放入scheduler的id_cr中
  * 2将该协程放入classic_context的多优先级队列cr_group中
  * 3唤醒一个协程运行该func(task) !!
  */
  auto task_id = GlobalData::RegisterTaskName(name);

  auto cr = std::make_shared<CRoutine>(func); //定义协程对象，为其绑定协程函数和上下文context
  cr->set_id(task_id);
  cr->set_name(name);
...
void Scheduler::ProcessLevelResourceControl() {
  ...
  for (const auto cpu : cpus) {
    CPU_SET(cpu, &set);  // 将对应的cpu的掩码置位
  }
  /*
  * pthread_self() 获得当前线程的id
  * pthread_setaffinity_np　将该线程限制为只在set指定的cpu上运行
  */
