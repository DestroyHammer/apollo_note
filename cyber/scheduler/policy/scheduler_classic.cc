...
void SchedulerClassic::CreateProcessor() {
  for (auto& group : classic_conf_.groups()) {
    ...
    for (uint32_t i = 0; i < proc_num; i++) {
      // 初始化一个context对象，压入processorcontext队列
      auto ctx = std::make_shared<ClassicContext>(group_name);
      pctxs_.emplace_back(ctx);
      /*
      * 初始化processor对象，为其绑定context配置
      * 和一个运行processor::run()的线程
      * 在run中获取优先级最高的协程，并resume
      * 为该线程设置cpu affinity
      * 为该线程设置调度策略和优先级
      */
...
bool SchedulerClassic::DispatchTask(const std::shared_ptr<CRoutine>& cr) {
...

  {
    ...
    if (id_cr_.find(cr->id()) != id_cr_.end()) {
      return false;  // 要分发的协程cr已在调度器中，返回false
    }
    id_cr_[cr->id()] = cr;  // 否则将协程cr加入到调度器
  }
  // 根据配置为协程设置优先级，默认为0
  ...
  // Enqueue task.  将协程放入优先级队列
  {
    WriteLockGuard<AtomicRWLock> lk(
        ClassicContext::rq_locks_[cr->group_name()].at(cr->priority()));
    ClassicContext::cr_group_[cr->group_name()]
        .at(cr->priority())
        .emplace_back(cr);  // 将协程添加到真正用于调度的优先级队列cr_group_中，区别于id_cr_
  }

  ClassicContext::Notify(cr->group_name());  // 通过信号量唤醒ClassicContext::wait !!
  return true;
}
