...
bool BoundedQueue<T>::Init(uint64_t size, WaitStrategy* strategy) {
  ...
  // 为任务队列每个元素初始化一个空的std::function对象
  for (uint64_t i = 0; i < pool_size_; ++i) {
    new (&(pool_[i])) T();  //为队列中每个成员分配空间
  }
  // 设置等待策略
...
bool BoundedQueue<T>::Enqueue(const T& element) {
  ...
  do {
    ...
    if (GetIndex(new_tail) == GetIndex(head_.load(std::memory_order_acquire))) {
      return false; //队列满了，插入失败
    }
    //检查tail_是否等于old_tail，保证线程安全
    //若相等，tail_值更新为new_tail，返回true跳出循环
    //若不等，old_tail更新为new_tail,返回false
  } while (!tail_.compare_exchange_weak(old_tail, new_tail,
                                        std::memory_order_acq_rel,
                                        std::memory_order_relaxed));
  pool_[GetIndex(old_tail)] = element; //old_tail指向真正的待插入的位置
  do {
    old_commit = old_tail;
  } while (cyber_unlikely(!commit_.compare_exchange_weak(
      old_commit, new_tail, std::memory_order_acq_rel,
      std::memory_order_relaxed))); //若commit_与old_commit不等时，说明new_tail、old_tail都变了 commit_作用是什么???? 保证访问tail_的线程安全
...
bool BoundedQueue<T>::Dequeue(T* element) {
  ...
  do {
    ...
    if (new_head == commit_.load(std::memory_order_acquire)) {
      return false;  //空队列，出队失败  ////为什么以commit判断而不是tail? 为了保证线程安全
    }
